-- // Coded by _notportal (Modified for compatibility)
-- // VisionWare GUI - Fixed visibility issues
		...
		function Library:Resize(object, background)
			...
			Library:Connection(Mouse.Move, function(input)
				if dragging then
					...
					local Y = math.clamp((MouseLocation.Y - 36) - background.AbsolutePosition.Y, 400, 9999)
	...
	do
		function Library:NewPicker(name, default, defaultalpha, parent, count, flag, callback)
			...
			Icon.MouseButton1Down:Connect(function()
				...
				-- Render the picker above the whole GUI so it never clips inside a section
				local Screen = Library.ScreenGUI
				if Screen and ColorWindow.Parent ~= Screen then
					ColorWindow.Parent = Screen
				end
				local ScreenSize = Screen and Screen.AbsoluteSize or Vector2.new(1920, 1080)
				local IconPos = Icon.AbsolutePosition
				local IconSize = Icon.AbsoluteSize
				local WinSize = ColorWindow.AbsoluteSize
				ColorWindow.ZIndex = 99999999
				ColorWindow.AnchorPoint = Vector2.new(0, 0)
				local MaxX = math.max(ScreenSize.X - WinSize.X - 4, 4)
				local MaxY = math.max(ScreenSize.Y - WinSize.Y - 4, 4)
				local NewX
				if IconPos.X > ScreenSize.X / 2 then
					NewX = math.clamp(IconPos.X - WinSize.X - 4, 4, MaxX)
				else
					NewX = math.clamp(IconPos.X + IconSize.X + 4, 4, MaxX)
				end
				local NewY = math.clamp(IconPos.Y, 4, MaxY)
				ColorWindow.Position = UDim2.new(0, NewX, 0, NewY)
			end)
	...
	do
		...
		function Library:Window(Options)
			local Window = {
				...
				Name = (Options.Name or Options.name or "VisionWare");
			};
			--
			-- FIX: Better GUI parent selection
			local UI
			...
			if not success or not UI then
				...
				pcall(function()
					UI = Instance.new("ScreenGui", game.Players.LocalPlayer:WaitForChild("PlayerGui"))
				end)
			end
			
			-- If both fail, create it without parent and set later
			if not UI then
			...
			AccentOutline.AnchorPoint = Vector2.new(0.5, 0.5) -- Center it
			...
			AccentOutline.Position = UDim2.new(0.5, -316, 0.5, -206) -- Centered position
			AccentOutline.Size = UDim2.new(0, 631, 0, 411)
			...
			PageInline.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
			...
			TopAccent.BorderColor3 = Color3.fromRGB(0, 0, 0)
		...
		function Library:Page(Properties)
			...
			Left.BottomImage = "rbxassetid://7783554086"
			Left.CanvasSize = UDim2.new()
			Left.MidImage = "rbxassetid://7783554086"
			Left.ScrollBarImageColor3 = Library.Accent
			Left.ScrollBarThickness = 6
			Left.TopImage = "rbxassetid://7783554086"
			...
			Right.BottomImage = "rbxassetid://7783554086"
			Right.CanvasSize = UDim2.new()
			Right.MidImage = "rbxassetid://7783554086"
			Right.ScrollBarImageColor3 = Library.Accent
			Right.ScrollBarThickness = 6
			Right.TopImage = "rbxassetid://7783554086"
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "VisionWare | v1.0");
			}
			--
			local Outline = Instance.new("Frame")
			Outline.Name = "Outline"
		...
		function Library:Indicator(Properties)
			local Indicator = {
				Title = Properties.Name or Properties.name or "VisionWare Indicator";
				Elements = {};
				Dragging = { false, UDim2.new(0, 0, 0, 0) };
			};
			--
...
-- Create the GUI
local Window = Library:Window({Name = "VisionWare v1.0", Amount = 6})
local Watermark = Library:Watermark({Name = "VisionWare | v1.0"})
...
-- ===== CONFIG HELPERS =====
-- Uses the executor filesystem when available; safe no-op otherwise.
local function GetConfigFiles()
	local files = {}
	pcall(function()
		if not listfiles then return end
		for _, f in ipairs(listfiles("VisionWare")) do
...
local function SaveConfig()
	local name = tostring(Library.Flags.Settings_ConfigName or "")
	if name == "" then name = "config" end
	pcall(function()
		if not writefile then return end
		writefile("VisionWare/" .. name .. ".txt", Library:GetConfig())
	end)
	Library:Notification(("Saved config \"%s\""):format(name), 3, Library.Accent)
end
...
local function LoadConfig(name)
	...
	local ok = pcall(function()
		...
		Library:LoadConfig(readfile("VisionWare/" .. name .. ".txt"))
	end)
	if ok then Library:Notification(("Loaded config \"%s\""):format(name), 3, Library.Accent) end
end
...
local function DeleteConfig(name)
	...
	pcall(function()
		if delfile then
			delfile("VisionWare/" .. name .. ".txt")
		elseif os.remove then
			os.remove("VisionWare/" .. name .. ".txt")
		end
	end)
	Library:Notification(("Deleted config \"%s\""):format(name), 3, Library.Accent)
end
...
Aimbot:List({Name = "Hitpart", Flag = "Aimbot_Hitpart", Options = {"Head", "HumanoidRootPart", "Torso"}, Default = "Head"})
Aimbot:Keybind({Name = "Activation Key", Flag = "Aimbot_Key", Mode = "Hold", Default = Enum.UserInputType.MouseButton2})
Aimbot:Slider({Name = "Smoothness", Flag = "Aimbot_Smoothness", Min = 0.01, Max = 1, Default = 0.2, Decimals = 0.01})
Aimbot:Toggle({Name = "Use FoV", Flag = "Aimbot_FoV", Default = true})
Aimbot:Slider({Name = "FoV Size", Flag = "Aimbot_FoVSize", Min = 1, Max = 200, Default = 50, Suffix = "°", Decimals = 1})
Aimbot:Colorpicker({Name = "FoV Color", Flag = "Aimbot_FoVColor", Default = Library.Accent})
Aimbot:Toggle({Name = "Prediction", Flag = "Aimbot_Prediction"})
Aimbot:Slider({Name = "Prediction Amount", Flag = "Aimbot_PredAmount", Min = 0, Max = 1, Default = 0.25, Decimals = 0.01})
Aimbot:Toggle({Name = "Visible Check", Flag = "Aimbot_VisibleOnly"})
Aimbot:Toggle({Name = "Team Check", Flag = "Aimbot_TeamCheck", Default = true})
...
Trigger:Slider({Name = "Silent FoV Size", Flag = "SilentAim_FoVSize", Min = 1, Max = 200, Default = 80, Suffix = "°", Decimals = 1})
Trigger:Colorpicker({Name = "Silent FoV Color", Flag = "SilentAim_FoVColor", Default = Library.Accent})
Trigger:Slider({Name = "Hit Chance", Flag = "SilentAim_HitChance", Min = 0, Max = 100, Default = 100, Suffix = "%", Decimals = 1})
Trigger:Toggle({Name = "Show Silent Target", Flag = "SilentAim_ShowTarget"})
Trigger:Toggle({Name = "Silent Prediction", Flag = "SilentAim_Prediction"})
Trigger:Slider({Name = "Prediction Amount", Flag = "SilentAim_PredAmount", Min = 0.165, Max = 1, Default = 0.165, Decimals = 0.001})
...
local ESP2 = ESPPage:Section({Name = "Colors / Filters", Side = "Right"})

ESP:Toggle({Name = "Enable ESP", Flag = "ESP_Enabled", Default = true})
ESP:Toggle({Name = "Team Check", Flag = "ESP_TeamCheck"})
ESP:Toggle({Name = "Show Team", Flag = "ESP_ShowTeam"})
ESP:Toggle({Name = "Boxes", Flag = "ESP_Boxes", Default = true})
ESP:List({Name = "Box Type", Flag = "ESP_BoxType", Options = {"2D Box", "3D Box", "Corner Box", "Filled Box"}, Default = "2D Box"})
ESP:Slider({Name = "Box Thickness", Flag = "ESP_BoxThickness", Min = 1, Max = 5, Default = 1, Suffix = "px", Decimals = 1})
ESP:Toggle({Name = "Outline", Flag = "ESP_Outline", Default = true})
ESP:Toggle({Name = "Names", Flag = "ESP_Names", Default = true})
ESP:List({Name = "Name Mode", Flag = "ESP_NameMode", Options = {"DisplayName", "Username"}, Default = "DisplayName"})
ESP:Slider({Name = "Text Size", Flag = "ESP_TextSize", Min = 10, Max = 24, Default = 13, Suffix = "px", Decimals = 1})
ESP:Toggle({Name = "Health Bar", Flag = "ESP_Health", Default = true})
ESP:List({Name = "Health Style", Flag = "ESP_HealthStyle", Options = {"Bar", "Text", "Both"}, Default = "Both"})
ESP:List({Name = "Health Bar Side", Flag = "ESP_HealthBarSide", Options = {"Left", "Right"}, Default = "Left"})
ESP:Toggle({Name = "Distance", Flag = "ESP_Distance"})
ESP:Toggle({Name = "Tracers", Flag = "ESP_Tracers", Default = true})
ESP:List({Name = "Tracer Type", Flag = "ESP_TracerType", Options = {"From Bottom", "From Top", "From Center", "From Mouse"}, Default = "From Bottom"})
ESP:Slider({Name = "Tracer Thickness", Flag = "ESP_TracerThickness", Min = 1, Max = 4, Default = 1, Suffix = "px", Decimals = 1})
ESP:Toggle({Name = "Skeleton", Flag = "ESP_Skeleton"})
ESP:Slider({Name = "Skeleton Thickness", Flag = "ESP_SkeletonThickness", Min = 1, Max = 3, Default = 1, Suffix = "px", Decimals = 1})
ESP:Toggle({Name = "Head Dot", Flag = "ESP_Head"})
ESP:Toggle({Name = "Chams", Flag = "ESP_Chams", Default = true})
ESP:Slider({Name = "ESP Range", Flag = "ESP_Range", Min = 10, Max = 1000, Default = 500, Suffix = " studs", Decimals = 1})

ESP2:Colorpicker({Name = "Enemy Color", Flag = "ESP_EnemyColor", Default = Library.Accent})
ESP2:Colorpicker({Name = "Team Color", Flag = "ESP_TeamColor", Default = Color3.fromRGB(86, 227, 120)})
ESP2:Colorpicker({Name = "Chams Color", Flag = "ESP_ChamsColor", Default = Color3.fromRGB(255, 255, 255)})
ESP2:Colorpicker({Name = "Health Color", Flag = "ESP_HealthColor", Default = Color3.fromRGB(0, 255, 0)})
ESP2:Colorpicker({Name = "Tracer Color", Flag = "ESP_TracerColor", Default = Library.Accent})
ESP2:Colorpicker({Name = "Skeleton Color", Flag = "ESP_SkeletonColor", Default = Color3.fromRGB(255, 255, 255)})
ESP2:Slider({Name = "Opacity", Flag = "ESP_Opacity", Min = 0, Max = 100, Default = 75, Suffix = "%", Decimals = 1})
ESP2:Toggle({Name = "Visible Only", Flag = "ESP_VisibleOnly"})
ESP2:Toggle({Name = "Status Overlay", Flag = "ESP_Status", Default = true})
ESP2:Toggle({Name = "Rainbow", Flag = "ESP_Rainbow"})
ESP2:Slider({Name = "Rainbow Speed", Flag = "ESP_RainbowSpeed", Min = 0.1, Max = 5, Default = 1, Decimals = 1})
ESP2:List({Name = "Rainbow Parts", Flag = "ESP_RainbowParts", Options = {"All", "Boxes", "Tracers", "Text"}, Default = "All"})

-- ===== VISUALS =====
local Visuals = VisualsPage:Section({Name = "World / Lighting"})
local Visuals2 = VisualsPage:Section({Name = "Player / Effects", Side = "Right"})

Visuals:Toggle({Name = "Fullbright", Flag = "Visuals_Fullbright", Default = true})
Visuals:Toggle({Name = "Custom Brightness", Flag = "Visuals_Brightness"})
Visuals:Slider({Name = "Brightness", Flag = "Visuals_BrightnessAmount", Min = 0, Max = 10, Default = 3, Decimals = 0.1})
Visuals:Toggle({Name = "Remove Fog", Flag = "Visuals_NoFog"})
Visuals:Toggle({Name = "Custom Skybox", Flag = "Visuals_Skybox"})
Visuals:Toggle({Name = "No Clouds", Flag = "Visuals_Clouds"})
Visuals:Slider({Name = "Ambient Brightness", Flag = "Visuals_Ambient", Min = 0, Max = 10, Default = 3, Decimals = 0.1})

Visuals2:Toggle({Name = "Highlight", Flag = "Visuals_Highlight"})
Visuals2:Colorpicker({Name = "Highlight Color", Flag = "Visuals_HighlightColor", Default = Library.Accent})
Visuals2:Toggle({Name = "Night Vision", Flag = "Visuals_NightVision"})
Visuals2:Colorpicker({Name = "Night Vision Color", Flag = "Visuals_NightVisionColor", Default = Color3.fromRGB(60, 255, 120)})

-- ===== MOVEMENT =====
local Move = MovementPage:Section({Name = "Basic"})
local Move2 = MovementPage:Section({Name = "Advanced", Side = "Right"})

Move:Toggle({Name = "WalkSpeed", Flag = "Misc_Speed"})
Move:Slider({Name = "Speed", Flag = "Misc_SpeedAmount", Min = 16, Max = 300, Default = 80, Decimals = 1})
Move:Toggle({Name = "Jump Power", Flag = "Misc_Jump"})
Move:Slider({Name = "Jump", Flag = "Misc_JumpPower", Min = 50, Max = 500, Default = 100, Decimals = 1})
Move:Toggle({Name = "Infinite Jump", Flag = "Misc_JumpInfinite"})
Move:Toggle({Name = "Noclip", Flag = "Misc_Noclip"})

Move2:Toggle({Name = "Fly", Flag = "Misc_Fly"})
Move2:Slider({Name = "Fly Speed", Flag = "Misc_FlySpeed", Min = 1, Max = 200, Default = 50, Decimals = 1})
Move2:Keybind({Name = "Fly Key", Flag = "Misc_FlyKey", Mode = "Hold"})
Move2:Toggle({Name = "Strafe", Flag = "Misc_Strafe"})
Move2:Toggle({Name = "Velocity", Flag = "Misc_Velocity"})
Move2:Slider({Name = "Velocity Multiplier", Flag = "Misc_VelocityAmount", Min = 0, Max = 10, Default = 1, Decimals = 0.5})

-- ===== MISC =====
local Misc = MiscPage:Section({Name = "Utility"})
local Misc2 = MiscPage:Section({Name = "Extras", Side = "Right"})

Misc:Toggle({Name = "Anti-AFK", Flag = "Misc_AntiAFK", Default = true})
Misc:Toggle({Name = "Chat Spam", Flag = "Misc_ChatSpam"})
Misc:Textbox({Name = "Spam Message", Flag = "Misc_SpamText", Placeholder = "Message..."})
Misc:Slider({Name = "Spam Delay", Flag = "Misc_SpamDelay", Min = 0.5, Max = 10, Default = 2, Decimals = 0.5})
Misc:Toggle({Name = "FPS Mode", Flag = "Misc_FPSMode"})
Misc:Toggle({Name = "Panic (Master Toggle)", Flag = "Misc_Panic"})

Misc2:Button({Name = "Server Hop", Callback = function()
	pcall(function()
		game:GetService("TeleportService"):Teleport(game.PlaceId)
	end)
end})
Misc2:Button({Name = "Test Notification", Callback = function() Library:Notification("VisionWare Loaded!", 4, Library.Accent) end})

-- ===== SETTINGS =====
do
	local CFG = SettingsPage:Section({Name = "Menu"})
	local CFG2 = SettingsPage:Section({Name = "Configuration", Side = "Right"})

	CFG:Keybind({Name = "Menu Key", Flag = "Settings_MenuKey", UseKey = true, Default = Enum.KeyCode.End, Callback = function(Key) if Key then Library.UIKey = Key end end})
	CFG:Colorpicker({Name = "Menu Accent", Flag = "Settings_Accent", Default = Library.Accent, Callback = function(Color) Library:ChangeAccent(Color) end})
	CFG:Toggle({Name = "Show Watermark", Flag = "Settings_Watermark", Callback = function(v) Watermark:SetVisible(v) end})
	CFG:Textbox({Name = "Watermark Text", Flag = "Settings_WatermarkText", State = "VisionWare | v1.0", Callback = function(t) Watermark:UpdateText(t) end})

	local ConfigList = CFG2:List({Name = "Config List", Flag = "Settings_ConfigList", Options = {}, Callback = function(name) if name then LoadConfig(name) end end})
	CFG2:Textbox({Name = "Config Name", Flag = "Settings_ConfigName", Placeholder = "myconfig"})
	CFG2:Button({Name = "Save Config", Callback = function()
		SaveConfig()
		ConfigList:Refresh(GetConfigFiles())
	end})
	CFG2:Button({Name = "Load Config", Callback = function()
		LoadConfig(Library.Flags.Settings_ConfigList)
	end})
	CFG2:Button({Name = "Delete Config", Callback = function()
		DeleteConfig(Library.Flags.Settings_ConfigList)
		ConfigList:Refresh(GetConfigFiles())
	end})
end
...
-- ===== FOV RING RENDERER =====
do
	-- Aimbot / Silent Aim FoV circles drawn on top of the world.
	-- Uses the executor Drawing API when available, otherwise a UI fallback.
	local Circles = {}

	local function GetCircle(Key)
		if not Circles[Key] then
			if Drawing then
				local ok, c = pcall(Drawing.new, "Circle")
				if ok and c then
					Circles[Key] = { drawing = true, obj = c }
				end
			end
			if not Circles[Key] then
				local f = Instance.new("Frame")
				f.Name = "VisionFov_" .. Key
				...
				Stroke.Thickness = 2
				Stroke.Parent = f
				f.Parent = Library.ScreenGUI
				Circles[Key] = { drawing = false, obj = f, stroke = Stroke }
			end
		end
		return Circles[Key]
	end

	local function UpdateCircle(Key, Visible, Center, Radius, Color)
		local circle = GetCircle(Key)
		if circle.drawing then
			circle.obj.Visible = Visible
			if Visible then
				circle.obj.Color = Color
				circle.obj.Position = Center
				circle.obj.Radius = Radius
				circle.obj.Thickness = 1.5
				circle.obj.Filled = false
				circle.obj.Transparency = 1
			end
		else
			circle.obj.Visible = Visible
			if Visible then
				circle.obj.BackgroundColor3 = Color
				circle.obj.Size = UDim2.new(0, Radius * 2, 0, Radius * 2)
				circle.obj.Position = UDim2.new(0, Center.X, 0, Center.Y)
	...
	local function DrawFov(Key, ToggleFlag, SizeFlag, ColorFlag)
		local Enabled = not Library.Flags.Misc_Panic and Library.Flags[ToggleFlag]
		if Enabled then
			local Camera = workspace.CurrentCamera
			local Viewport = Camera and Camera.ViewportSize or Vector2.new(1920, 1080)
			local CamFov = Camera and Camera.FieldOfView or 70
			local Center = Vector2.new(Viewport.X / 2, Viewport.Y / 2)
			UpdateCircle(
				Key,
				true,
				Center,
				FovScreenRadius(Library.Flags[SizeFlag] or 50, Viewport.Y, CamFov),
				Library.Flags[ColorFlag] or Library.Accent
			)
		else
			UpdateCircle(Key, false, Vector2.new(0, 0), 0, Library.Accent)
		end
	end

	game:GetService("RunService").RenderStepped:Connect(function()
		DrawFov("Aimbot", "Aimbot_FoV", "Aimbot_FoVSize", "Aimbot_FoVColor")
		DrawFov("Silent", "SilentAim_FoV", "SilentAim_FoVSize", "SilentAim_FoVColor")
	end)
end
